...
half3 LightingCustom(BRDFData brdfData, BRDFData brdfDataClearCoat,
                                    ...
                                    half clearCoatMask, bool specularHighlightsOff, out half3 radiance)
{
    ...
    half radLength = length(radiance);
    half random = rand_1_05(half2(radLength, 1 - radLength));

    if(radLength > 0){
        return brdf * lightColor; // * random;
    }
    return 0;
}

half3 LightingCustom(BRDFData brdfData, BRDFData brdfDataClearCoat, Light light, half3 normalWS,
                                    half3 viewDirectionWS, half clearCoatMask, bool specularHighlightsOff, out half3 radiance)
{
    return LightingCustom(brdfData, brdfDataClearCoat, light.color, light.direction,
                                         ...
                                         clearCoatMask, specularHighlightsOff, radiance);
}

half4 UniversalFragmentCustom(InputData inputData, SurfaceData surfaceData, Varyings input)
{
    ...
    color += LightingCustom(brdfData, brdfDataClearCoat,
                                           mainLight,
                                           inputData.normalWS, inputData.viewDirectionWS,
                                           surfaceData.clearCoatMask, specularHighlightsOff, radiance);

    //#ifdef _ADDITIONAL_LIGHTS
    uint pixelLightCount = GetAdditionalLightsCount();

    half3 additionalLightColor = 0;
    uint effectiveLightCount=0;
    ...
    for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
    {
        ...
        half3 lightColorRadiance;
        half3 lightColor = LightingCustom(brdfData, brdfDataClearCoat,
                                         light,
                                         inputData.normalWS, inputData.viewDirectionWS,
                                         surfaceData.clearCoatMask, specularHighlightsOff, lightColorRadiance);

        if(length(lightColor) > 0.0001){
            
            totalRadiance += pow(lightColorRadiance, 2);

            if (effectiveLightCount == 0)
            {
                additionalLightColor = lightColor;
            }
            else
            {
                //additionalLightColor = lerp(additionalLightColor, lightColor, saturate(pow(lightColorRadiance / (pow(lightColorRadiance, 2) + pow(lastRadiance, 2)), 3)));
                
                half3 oldLight = additionalLightColor;
                half3 newLight = lightColor;
                half3 lerpvalue = lightColorRadiance / sqrt(totalRadiance);
                
                half3 result = lerp(oldLight, newLight, lerpvalue);
                
                additionalLightColor = result;
    ...
    if(effectiveLightCount > 0){
        //additionalLightColor *= effectiveLightCount;
    }
    //additionalLightColor = normalize(additionalLightColor);
...
half4 frag(PackedVaryings packedInput) : SV_TARGET
{
    ...
    half4 color = UniversalFragmentCustom(inputData, surface, unpacked);
